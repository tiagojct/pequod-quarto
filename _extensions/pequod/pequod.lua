...
-- Two jobs:
--
-- 1. Forward `pequod.logo` metadata to the format-specific keys that
...
--
-- Conservative by design: existing user-set values are NEVER
-- overwritten, and meta-tag emission is skipped for any field the
-- user did not declare.

local function get_pequod_logo(meta)
  if meta.pequod == nil then return nil end
  if type(meta.pequod) == 'table' and meta.pequod.logo ~= nil then
    return meta.pequod.logo
  end
  return nil
end
...
-- suitable for an HTML attribute. Falls back to nil so callers can
-- skip emitting attribute = "" tags.
...
-- Comma-join authors. Pandoc accepts either a single string or a list
-- of {name = ..., affiliation = ...} maps; handle both.
...
-- Resolve an OG image URL in order:
--   1. meta.pequod['og-image']                (extension key)
--   2. meta.website['open-graph'].image       (Quarto's native key)
--   3. meta['og-image']                       (per-page override)
-- Absence of og:image is acceptable per protocol.
local function get_og_image(meta)
  if meta.pequod ~= nil and type(meta.pequod) == 'table' and meta.pequod['og-image'] ~= nil then
    return meta_to_string(meta.pequod['og-image'])
  end
...
function Meta(meta)
  local logo = get_pequod_logo(meta)
  if logo ~= nil then
